...
def build_graph_all(project_id: str):
    ...
    Used when everything is already committed and you just need the graph built.
    """
    repo      = get_repo(project_id)
    errors    = []
    processed = []
    ...
    skip_exts  = {'.pyc', '.pyo', '.out', '.log', '.bin', '.so', '.o', '.a'}
    ...
    for f in repo.repo_path.rglob("*"):
        ...
        if any(part in skip_dirs or part.startswith('.') for part in f.parts[len(repo.repo_path.parts):]):
            continue
    ...
    try:
        ...
        if repo.repo.is_dirty(index=True):
            repo.repo.index.commit(f"Built graph from {len(processed)} files")
    except Exception as e:
